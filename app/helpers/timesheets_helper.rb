    ...
    html = ''
      ...
      if !entry.manual?
        ...
        if status=='new'
          ...
          html << "<a href='#' onclick=\"new Ajax.Updater('screen_body', '/timesheets/update/#{id}?commit=add&selected_date=#{entry.dateValue}&rate_id=#{entry.rate_id}', {loading:Element.show('loader_#{entry.id}'),asynchronous:true, evalScripts:true,parameters:Form.serialize('edit_timesheet_#{id}')});return false;\""
         end
         ...
         if status=='new'
          ...
          html << "><img src=#{path_to_image('add_blue.png')} name = 'add_d' style='top:4px' alt='' class=/'icon_48/' ></a>"
      end
      end

    if entry.manual?
      if status == 'new'
        html << "<a href='#' onclick=\"new Ajax.Updater('screen_body', '/timesheets/create/#{id}?commit=remove&#{count}_remove.x', {loading:Element.show('loader_#{count}'),asynchronous:true, evalScripts:true,parameters:Form.serialize('new_timesheet')});return false;\""
        html << "><img src='/images/cross_48.gif' class='small_icon' alt='' style='size:16*16'  ></a>"
#        html << delete_shift_button(entries.index(entry).to_s)
      else
        html << "<a href='#' onclick=\"new Ajax.Updater('screen_body', '/timesheets/update/#{id}?commit=remove&#{entry.id}_remove.x', {loading:Element.show('loader_#{entry.id}'),asynchronous:true, evalScripts:true,parameters:Form.serialize('edit_timesheet_#{id}')});return false;\""
        html << "><img src='/images/cross_48.gif' class='small_icon' alt='' style='size:16*16'  ></a>"
      end
    end
    return html
